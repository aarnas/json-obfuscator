...
export function obfuscate(inputJSON, mappingFileName) {
    ...
    if (Array.isArray(jsonValues)) {
        for (var obfuscatedJson = String(inputJSON), result = "", d = 0; d < jsonValues.length;) {
            obfuscatedJson = obfuscatedJson.replace(jsonValues[d].replace('"', "").replace('"', ""), "" + unicodeEscape(jsonValues[d].replace('"', "").replace('"', "")))
        ...
        fs.writeFile(`./output/${mappingFileName}.txt`, mapping.join("\n").toString(), err => {
            if (err) {
                console.error(err)
                return
            }
